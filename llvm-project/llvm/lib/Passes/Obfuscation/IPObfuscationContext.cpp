...
namespace llvm
{
    ...
    Function *IPObfuscationContext::InsertSecretArgument(Function *F)
    {
        ...
        Params.push_back(PointerType::get(F->getContext(), 0));
...
// Reconstruct the AttributesList based on the vector we constructed.
        ...
        while (!F->use_empty())
        {
            ...
            for (User *U : F->users())
            {
                if (auto *DirectCB = dyn_cast<CallBase>(U))
                {
                    CB = DirectCB;
                    break;
                }
                ...
                {
                    auto *MaybeCB = dyn_cast<CallBase>(CEUser);
                    if (!MaybeCB)
                        continue;
                    if (MaybeCB->getCalledOperand() == CE)
                    {
                        CB = MaybeCB;
                        break;
                    }
...
// Reconstruct the AttributesList based on the vector we constructed.
        ...
        NF->splice(NF->begin(), F, F->begin(), F->end());
        ...
        LoadInst *MySecret = IRB.CreateLoad(I32Ty, Ptr);
